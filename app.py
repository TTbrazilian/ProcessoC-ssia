...
# ----------------------------------------------------------------------------
# Diagnóstico (barra lateral) — testa Sheets, Drive e e-mail separadamente
# ----------------------------------------------------------------------------
def testar_drive():
    if "gcp_service_account" not in st.secrets:
        return False, "Secrets não carregados."
    folder_id = st.secrets.get("drive", {}).get("folder_id", "")
    if not folder_id:
        return False, "folder_id não configurado em [drive]."
    try:
        service = _drive_service()
        meta = service.files().get(
            fileId=folder_id, fields="id,name,mimeType", supportsAllDrives=True).execute()
        return True, f"Pasta '{meta.get('name')}' acessível."
    except Exception as e:
        return False, f"{type(e).__name__}: {e}"
...
def _dica(msg):
    m = msg or ""
    if "has not been used" in m or "is disabled" in m or "accessNotConfigured" in m:
        return "→ Ative a **Google Drive API** no projeto e aguarde 1–2 min."
    if "File not found" in m or "notFound" in m or "404" in m:
        return "→ Compartilhe a **pasta do Drive** com a conta de serviço como **Editor**."
    if "storageQuota" in m or "quota" in m.lower():
        return "→ Conta pessoal: use um **Drive Compartilhado** (Workspace) para os anexos."
    if "must not be an Office file" in m:
        return "→ A planilha precisa ser **Google Sheets nativa**, não um Excel enviado."
    if "Authentication" in m or "auth" in m.lower() or "535" in m:
        return "→ Verifique o **usuario** e a **SMTP key** do Brevo."
    if "not verified" in m.lower() or "sender" in m.lower() or "550" in m:
        return "→ Verifique o **remetente** (Sender) no painel do Brevo."
    return ""


with st.sidebar:
    st.markdown("### 🔧 Diagnóstico")
    st.caption("Use para validar as integrações. Remova depois de configurar tudo.")

    ok_sec = "gcp_service_account" in st.secrets
    st.write(("✅" if ok_sec else "❌") + " Secrets carregados")
    try:
        import gspread  # type: ignore  # noqa
        from googleapiclient.discovery import build as _b  # type: ignore  # noqa
        st.write("✅ Bibliotecas Google instaladas")
    except ImportError:
        st.write("❌ Bibliotecas Google ausentes (gspread / google-api-python-client)")

    if ok_sec:
        st.caption("Conta de serviço (compartilhe planilha e pasta com este e-mail):")
        st.code(st.secrets["gcp_service_account"].get("client_email", "—"), language=None)

    if st.button("🔌 Testar Google Sheets"):
        ws, motivo = conectar_sheets()
        if ws is not None:
            st.success(f"Sheets OK: “{ws.spreadsheet.title}”.")
        else:
            st.error(motivo)
            if _dica(motivo):
                st.info(_dica(motivo))

    if st.button("📁 Testar Google Drive"):
        ok, info = testar_drive()
        (st.success if ok else st.error)(("Drive OK: " if ok else "") + info)
        if not ok and _dica(info):
            st.info(_dica(info))

    dest_teste = st.text_input("E-mail para teste",
                               value=st.secrets.get("email", {}).get("remetente", ""))
    if st.button("📧 Testar envio de e-mail"):
        ok, motivo = enviar_email_confirmacao(dest_teste, "Teste de Diagnóstico", "TESTE-0000")
        if ok:
            st.success(f"E-mail enviado para {dest_teste}. Confira a caixa de entrada/spam.")
        else:
            st.error(motivo)
            if _dica(motivo):
                st.info(_dica(motivo))
